...
public class MessageProcess {
	...
	public boolean queueMessage(String message) {
		if (messageQueue.size() >= server.getMaxRequest()) {
			System.out.println("Requests reached maximum (" + messageQueue.size() + ")");
			return false;
		}
		messageQueue.add(message);
		System.out.println("Queue message (" + messageQueue.size() + ")");
		return true;
	}
	...
	public void stop() {
		if (this.sendThread != null) {
			...
			this.segureStop.stop();
			this.segureStop = null;
...
//
	public void stopsecure() {
		this.segureStop = new Thread(new Runnable() {
			public void run() {
				try {
					while (true) {
						Thread.sleep(1000 * 10);
